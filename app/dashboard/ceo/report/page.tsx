        ...
        <div className="p-5 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {(selectedDate || selectedDept) && (
              <button 
                onClick={() => { setSelectedDate(''); setSelectedDept(''); }}
                        ...
                        <td className="py-4 px-6">
                          <div className="flex justify-center">
                            {(leave.status || '').toLowerCase().includes('approved') && (
                              <span className="bg-[#16a34a] text-white px-4 py-1.5 rounded-full text-[13px] font-bold">อนุมัติ</span>
                            )}
                            {(leave.status || '').toLowerCase().includes('rejected') && (
                              <span className="bg-[#ef4444] text-white px-4 py-1.5 rounded-full text-[13px] font-bold">ปฏิเสธ</span>
                            )}
                            {(leave.status || '').toLowerCase() === 'pending' && (
                              <span className="bg-[#f59e0b] text-white px-4 py-1.5 rounded-full text-[13px] font-bold">รออนุมัติ</span>
                            )}
                            {(leave.status || '').toLowerCase() === 'waiting ceo' && (
                              <span className="bg-[#6366f1] text-white px-4 py-1.5 rounded-full text-[13px] font-bold">รอ CEO อนุมัติ</span>
                            )}
                          </div>
                        </td>
                        <td className="py-4 px-6">
