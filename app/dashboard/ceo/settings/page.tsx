    ...
    const fetchProfile = async () => {
      try {
        ...
        if (res.success || res.data || res) {
          ...
          setDepartment(data.department?.name || data.departmentName || '-');
          setPosition(
            data.position?.title ||
              data.positionName ||
              data.position?.name ||
        ...
        <div className="bg-white/70 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl shadow-xl border border-white/50 dark:border-slate-800 p-8 max-w-[850px] mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out flex flex-col md:flex-row gap-12">
          ...
          <div className="flex flex-col items-center justify-start md:w-1/3 pt-4">
            <div className="relative group cursor-pointer mb-4">
              ...
              <label className="absolute bottom-2 right-2 w-10 h-10 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:shadow-blue-500/50 transition-all duration-300 border-[3px] border-white dark:border-slate-800 hover:scale-110 z-20">
            ...
            <div className="flex items-center gap-1.5 mt-1 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 px-3 py-1 rounded-full text-xs font-semibold shadow-sm border border-indigo-100 dark:border-indigo-800">
              <ShieldCheck className="w-3.5 h-3.5" /> CEO Role
            </div>
            {rawHireDate && (
