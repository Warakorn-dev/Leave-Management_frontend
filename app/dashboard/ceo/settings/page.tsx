    ...
    const fetchProfile = async () => {
      try {
        ...
        if (res.success || res.data || res) {
          ...
          setPosition(data.position?.title || data.positionName || data.position?.name || "-");
          ...
          const avatar = data.user?.avatarUrl || data.avatarUrl || data.profilePic || (typeof window !== 'undefined' ? sessionStorage.getItem('profilePic') : null);
      ...
      <div className="flex-1 p-6 md:p-8 z-10">
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-xl border border-white/50 p-8 max-w-[850px] mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out flex flex-col md:flex-row gap-12">
          ...
          <div className="flex flex-col items-center justify-start md:w-1/3 pt-4">
            ...
            <div className="flex items-center gap-1.5 mt-1 bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-semibold shadow-sm border border-indigo-100">
