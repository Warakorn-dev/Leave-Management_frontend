...
export default function RequestLeavePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (leaveMode === 'hourly') {
      ...
      const calculatedDays = calculateLeaveDays(startDate, endDate, p, 'full', holidaysData);
      if (calculatedDays <= 0) {
        setErrorMsg("จำนวนวันลาเป็น 0 (อาจตรงกับวันหยุดหรือเสาร์-อาทิตย์) กรุณาเลือกวันใหม่อีกครั้ง");
        setShowErrorModal(true);
        return;
      }
    }
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      if (leaveMode === 'hourly') {
        ...
        const p = leaveMode === 'half_day' ? period : 'full';
        payload.totalDays = calculateLeaveDays(startDate, endDate, p, 'full', holidaysData);
      }
                  ...
                  {[...balances].sort((a: any, b: any) => {
                    ...
                    return (
                      <option 
                        ...
                        className={isOutOfQuota ? "text-gray-400 bg-gray-50 font-medium" : "text-gray-800"}
                      >
                        {b.leaveType.name} {isOutOfQuota ? '(หมดโควต้า)' : (b.remainingDays <= 0 ? '(ใช้เกินโควต้า)' : `(เหลือ ${b.remainingDays} วัน)`)}
                      </option>
                    );
                  })}
                </select>
      ...
      {showErrorModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl max-w-[400px] w-full p-8 text-center animate-in zoom-in-95 duration-200 mx-4">
            ...
            <h2 className="text-xl font-bold text-black mb-3">เกิดข้อผิดพลาด</h2>
