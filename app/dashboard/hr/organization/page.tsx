...
import { Building2, Briefcase, Plus, Search, MoreVertical, Edit, Trash2, X, Users, User, Calculator, Shield, Megaphone, Headset, Monitor, FolderKanban, Crown, Code, PenTool, Award, Star } from 'lucide-react';
...
>>>>>>> ea55318422e9aa241b4f3765c0e062487f92721f
...
const getDepartmentStyle = (name: string) => {
  ...
  if (n.includes('human') || n.includes('hr') || n.includes('personnel')) return { icon: Users, colorClass: 'text-blue-600 dark:text-blue-400', bgClass: 'bg-blue-100 dark:bg-blue-900/30' };
  if (n.includes('account') || n.includes('finance')) return { icon: Calculator, colorClass: 'text-rose-600 dark:text-rose-400', bgClass: 'bg-rose-100 dark:bg-rose-900/30' };
  if (n.includes('admin')) return { icon: Shield, colorClass: 'text-slate-600 dark:text-slate-400', bgClass: 'bg-slate-200 dark:bg-slate-800' };
  if (n.includes('sale') || n.includes('market')) return { icon: Megaphone, colorClass: 'text-amber-600 dark:text-amber-400', bgClass: 'bg-amber-100 dark:bg-amber-900/30' };
  if (n.includes('support') || n.includes('service')) return { icon: Headset, colorClass: 'text-cyan-600 dark:text-cyan-400', bgClass: 'bg-cyan-100 dark:bg-cyan-900/30' };
  if (n.includes('it') || n.includes('information') || n.includes('tech')) return { icon: Monitor, colorClass: 'text-violet-600 dark:text-violet-400', bgClass: 'bg-violet-100 dark:bg-violet-900/30' };
  if (n.includes('project')) return { icon: FolderKanban, colorClass: 'text-emerald-600 dark:text-emerald-400', bgClass: 'bg-emerald-100 dark:bg-emerald-900/30' };
  return { icon: Building2, colorClass: 'text-indigo-600 dark:text-indigo-400', bgClass: 'bg-indigo-100 dark:bg-indigo-900/30' };
};

const getPositionStyle = (name: string) => {
  const n = name.toLowerCase();
  if (n.includes('ceo') || n.includes('director') || n.includes('chief') || n.includes('leader') || n.includes('manager') || n.includes('head')) return { icon: Crown, colorClass: 'text-amber-500 dark:text-amber-400', bgClass: 'bg-amber-100 dark:bg-amber-900/30' };
  if (n.includes('senior') || n.includes('sr')) return { icon: Star, colorClass: 'text-yellow-600 dark:text-yellow-400', bgClass: 'bg-yellow-100 dark:bg-yellow-900/30' };
  if (n.includes('dev') || n.includes('program') || n.includes('engineer')) return { icon: Code, colorClass: 'text-blue-600 dark:text-blue-400', bgClass: 'bg-blue-100 dark:bg-blue-900/30' };
  if (n.includes('design') || n.includes('graphic')) return { icon: PenTool, colorClass: 'text-fuchsia-600 dark:text-fuchsia-400', bgClass: 'bg-fuchsia-100 dark:bg-fuchsia-900/30' };
  if (n.includes('account') || n.includes('finance')) return { icon: Calculator, colorClass: 'text-rose-600 dark:text-rose-400', bgClass: 'bg-rose-100 dark:bg-rose-900/30' };
  if (n.includes('support') || n.includes('service') || n.includes('operator') || n.includes('sso')) return { icon: Headset, colorClass: 'text-cyan-600 dark:text-cyan-400', bgClass: 'bg-cyan-100 dark:bg-cyan-900/30' };
  if (n.includes('hr') || n.includes('human') || n.includes('personnel')) return { icon: Users, colorClass: 'text-violet-600 dark:text-violet-400', bgClass: 'bg-violet-100 dark:bg-violet-900/30' };
  return { icon: Briefcase, colorClass: 'text-emerald-600 dark:text-emerald-400', bgClass: 'bg-emerald-100 dark:bg-emerald-900/30' };
};
...
export default function OrganizationManagementPage() {
  ...
  const filteredPositions = useMemo(() => {
    let result = positions.filter(p => {
      const matchSearch = (p.name || p.title || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (p.departmentName || p.department?.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (p.code || '').toLowerCase().includes(searchTerm.toLowerCase());

      let matchDept = true;
      if (departmentFilter !== 'all') {
        const selectedDept = departments.find(d => d.id === departmentFilter);
        matchDept = p.departmentId === departmentFilter || (selectedDept && (p.departmentName || p.department?.name) === selectedDept.name) || false;
      }
  ...
  const getEmployeesInDept = (deptName: string, deptId: string) => {
    return employees.filter(emp => emp.departmentId === deptId || emp.departmentName === deptName);
  };
  ...
  const selectedDeptEmployees = useMemo(() => {
    ...
    return emps.sort((a, b) => {
      const getRank = (name: string) => {
        ...
        if (n.includes('leader') || n.includes('manager') || n.includes('director') || n.includes('head')) return 2;
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto pb-10 px-4 sm:px-6 md:px-8 pt-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <button
          onClick={() => activeTab === 'departments' ? setIsAddDeptModalOpen(true) : setIsAddPosModalOpen(true)}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden min-h-[400px]">
        {activeTab === 'departments' ? (
          <div className="p-6">
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-6">รายการแผนกทั้งหมด</h3>
            ...
            {isLoadingDept || isLoadingEmp || isLoadingPos ? (
              ...
              <div className="text-center py-10 text-slate-500">ไม่พบข้อมูลแผนก</div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className={`p-3 rounded-xl group-hover:scale-110 transition-transform ${style.bgClass} ${style.colorClass}`}>
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setEditDept({ id: dept.id, name: dept.name, code: dept.code || '', description: dept.description || '' });
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-sm">
                        <div className="flex flex-col">
                          ...
                          <span className="font-semibold text-slate-700 dark:text-slate-300">{deptEmps.length} คน</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-slate-400">ตำแหน่ง</span>
                          <span className="font-semibold text-slate-700 dark:text-slate-300">{deptPos.length} ตำแหน่ง</span>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {filteredPositions.length === 0 ? (
                    ...
                    filteredPositions.map((pos) => {
                      ...
                      const empsInPos = employees.filter(e => e.positionId === pos.id || e.positionName === pos.name);
                      const style = getPositionStyle(pos.name || pos.title || '');
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {selectedDeptEmployees.length === 0 ? (
                <div className="text-center py-10 flex flex-col items-center justify-center">
                  ...
                  <p className="text-slate-500 font-medium">ยังไม่มีพนักงานในแผนกนี้</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {selectedDeptEmployees.map(emp => (
                    <div key={emp.id} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {(() => {
                            const posStyle = getPositionStyle(emp.positionName || '');
                            const isLeader = (emp.positionName || '').toLowerCase().includes('leader') || (emp.positionName || '').toLowerCase().includes('ceo') || (emp.positionName || '').toLowerCase().includes('manager');
                            return (
                              <span className={`text-xs px-2 py-0.5 rounded-md font-medium border ${isLeader ? 'border-amber-200 dark:border-amber-800' : 'border-transparent'} ${posStyle.colorClass} ${posStyle.bgClass} truncate`}>
                                {emp.positionName || 'ไม่ระบุตำแหน่ง'}
                              </span>
                            );
                          })()}
                          <span className="text-xs text-slate-400 truncate ml-1">{emp.employeeId}</span>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">เพิ่มแผนกใหม่</h2>
                  <p className="text-sm text-slate-500">สร้างแผนกใหม่เพื่อจัดกลุ่มตำแหน่งงาน</p>
                </div>
              </div>
              <button onClick={() => setIsAddDeptModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-slate-300 rounded-full transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">ชื่อแผนก (Department Name) <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={newDept.name}
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">เพิ่มตำแหน่งใหม่</h2>
                  <p className="text-sm text-slate-500">สร้างตำแหน่งงานใหม่พร้อมระบุแผนกต้นสังกัด</p>
                </div>
              </div>
              <button onClick={() => setIsAddPosModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-slate-300 rounded-full transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">ชื่อตำแหน่ง (Position Name) <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={newPos.name}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">แผนกต้นสังกัด (Department) <span className="text-red-500">*</span></label>
                <select
                ...
                >
                  <option value="" disabled>-- เลือกแผนกต้นสังกัด --</option>
                  {departments.map(d => (
                    <option key={d.id} value={d.id}>{d.name}</option>
                  ))}
                </select>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  maxLength={5}
                  value={editDept.code}
                  onChange={(e) => setEditDept({ ...editDept, code: e.target.value.replace(/\D/g, '') })}
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all dark:text-white"
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  maxLength={5}
                  value={editPos.code}
                  onChange={(e) => setEditPos({ ...editPos, code: e.target.value.replace(/\D/g, '') })}
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all dark:text-white"
                  placeholder="เช่น 12345 (เว้นว่างไว้ระบบจะสุ่มให้)"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                  ชื่อตำแหน่ง (Position Title) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={editPos.name}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditPos({ ...editPos, departmentId: e.target.value })}
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all cursor-pointer dark:text-white"
                >
                  <option value="" disabled>-- เลือกแผนกต้นสังกัด --</option>
                  {departments.map(d => (
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">ยืนยันการลบ?</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm">
                คุณแน่ใจหรือไม่ว่าต้องการลบ <span className="font-semibold text-slate-700 dark:text-slate-300">{itemToDelete.name}</span> ?
                <br />
