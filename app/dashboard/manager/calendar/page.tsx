...
export default function LeaveCalendarPage() {
  ...
  interface CalendarDayEvent {
    ...
    isStart?: boolean;
    isMine?: boolean;
    raw?: CalendarLeaveEvent;
    date?: string;
  }

  const [allLeaves, setAllLeaves] = useState<CalendarLeaveEvent[]>([]);
  ...
  useEffect(() => {
    ...
    const apiLeaves = leavesData
      ...
      .map((leave) => ({
        ...
        type: (typeof leave.leaveType === 'object' ? leave.leaveType?.name : leave.leaveType) || leave.type,
        status: leave.status,
        name: leave.user?.firstName
          ? `${leave.user.title || ''}${leave.user.firstName} ${leave.user.lastName || ''}`.trim()
          : leave.employee?.firstName
            ? `${leave.employee.title || ''}${leave.employee.firstName} ${leave.employee.lastName || ''}`.trim()
            : leave.userId || 'User',
        departmentName:
        ...
        position:
          ...
          leave.employee?.positionName ||
          'พนักงาน',
        avatarUrl:
          leave.user?.avatarUrl || leave.employee?.user?.avatarUrl || null,
  ...
  const getEventsForDate = (day: number) => {
    ...
    const filteredLeaves = allLeaves.filter(
      (leave) => viewMode === 'company' || leave.departmentName === department,
    );
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#E2E4E9] font-sans text-slate-800 flex flex-col relative pb-8">
      ...
      <div className="flex-1 p-4 sm:p-6 md:p-8">
        <div className="max-w-[1200px] mx-auto bg-[#D9D9D9] p-4 rounded-xl shadow-md animate-in fade-in zoom-in-95 duration-300">
          ...
          {/* Calendar Grid */}
          <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-200 overflow-x-auto">
            <div className="grid grid-cols-7 w-full min-w-[700px] border-collapse">
              {/* Days Header */}
              {daysOfWeek.map((d) => (
                <div
                  key={d}
                  className="bg-white text-gray-500 font-bold text-center py-4 border-b border-r border-gray-200 text-[13px] tracking-wide last:border-r-0"
                >
                  {d}
                </div>
              ))}
              ...
              {blanks.map((b) => (
                <div
                  key={`blank-${b}`}
                  className="min-h-[140px] bg-white border-b border-r border-gray-200 p-2 last:border-r-0"
                ></div>
              ))}
                ...
                return (
                  ...
                  >
                    <div className="flex justify-end mb-1 px-1 pt-1">
                      <div
                        className={`w-7 h-7 flex items-center justify-center rounded-full text-[14px] font-bold ${isToday ? 'bg-[#1877F2] text-white shadow-md shadow-blue-500/30' : 'text-gray-700'}`}
                      >
                        {d}
                      </div>
                    </div>
                      ...
                      {events.slice(0, 2).map((evt, idx) => {
                        if (evt.type === 'holiday') {
                          return (
                            <div
                              key={idx}
                              className="bg-[#FFF0F0] text-[#FF0000] font-bold text-[11px] px-2.5 py-1.5 rounded-md truncate"
                            >
                              {evt.title}
                            </div>
                          );
                        ...
                        if (evt.type === 'leave') {
                          if (evt.isMine) {
                            return (
                              <div
                                key={idx}
                                className="bg-[#E6F7F8] text-[#12B8B8] font-bold text-[11px] px-2.5 py-1.5 rounded-md truncate border border-[#B2DFDB]"
                              >
                                {(evt.title || '').includes(' - ') ? (
                                  <>
                          ...
                          return (
                            <div
                              key={idx}
                              className="bg-[#F0F5FF] text-[#3B82F6] font-bold text-[11px] px-2.5 py-1.5 rounded-md truncate"
                            >
                              {(evt.title || '').includes(' - ') ? (
                                <>
                                  <span className="font-medium">
      ...
      {selectedDateEvents &&
        (() => {
          const leaveEvents = selectedDateEvents.events.filter(
            (e) => e.type === 'leave',
          );
          const holidayEvents = selectedDateEvents.events.filter(
            (e) => e.type === 'holiday',
          );
          ...
          const getInitials = (name: string) => {
            if (!name) return 'U';
            const parts = name
              .replace(/^(นาย|นาง|นางสาว|Mr\.|Ms\.|Mrs\.)/g, '')
              .trim()
              .split(' ');
            if (parts.length >= 2)
          ...
          const getBadgeStyle = (type: string) => {
            ...
            if (type.includes('พักร้อน') || type.includes('พักผ่อน'))
          ...
          const getTimeDisplay = (leave: CalendarLeaveEvent) => {
            if (
            ...
            if (leave.startFormat === 'morning')
              return 'ครึ่งวันเช้า (09:00 - 12:00)';
            if (leave.startFormat === 'afternoon')
              return 'ครึ่งวันบ่าย (13:00 - 17:00)';
            return 'เต็มวัน';
          };
          ...
          const dayOfWeek = new Date(
            selectedDateEvents.date,
          ).toLocaleDateString('th-TH', { weekday: 'long' });

          return (
            <div
              className="fixed inset-0 bg-black/40 z-[110] flex items-center justify-center backdrop-blur-sm p-4"
              onClick={() => setSelectedDateEvents(null)}
            >
              <div
                className="bg-white rounded-2xl shadow-xl w-full max-w-[500px] flex flex-col max-h-[90vh] overflow-hidden"
              ...
              >
                  ...
                  <div>
                    ...
                    {leaveEvents.length > 0 && (
                      <p className="text-sm font-medium text-slate-500 mt-1 flex items-center gap-2">
                        พนักงานลา {leaveEvents.length} คน
                      </p>
                    )}
                  </div>
                  <button
                ...
                {/* Body */}
                <div className="px-6 py-4 overflow-y-auto max-h-[600px] flex flex-col gap-4 bg-white">
                  {/* Holiday section */}
                  {holidayEvents.map((evt, idx: number) => (
                    <div
                      key={`holiday-${idx}`}
                  ...
                  {leaveEvents.length === 0 && holidayEvents.length === 0 && (
                    <div className="text-center py-12 flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm text-slate-500 mt-1">
                        ทุกคนพร้อมปฏิบัติงาน
                      </p>
                    </div>
                  )}

                  {/* Leave List */}
                  {leaveEvents.map((evt, idx: number) => {
                    const leave = evt.raw;
                    if (!leave) return null;
                    return (
                      <div
                        key={`leave-${idx}`}
                        className="flex flex-col gap-2 p-4 rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all bg-white group"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="w-[42px] h-[42px] rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-sm shrink-0 overflow-hidden relative">
                            ...
                            {leave.avatarUrl && (
                              ...
                              <img
                                src={
                                  leave.avatarUrl.startsWith('http')
                                    ? leave.avatarUrl
                                    : `/${leave.avatarUrl.replace(/^\/+/, '')}`
                                }
                                alt={leave.name}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1">
                              ...
                              <span className="text-[12px] font-bold text-slate-500 whitespace-nowrap bg-slate-100 px-2 py-0.5 rounded-md">
                                {getTimeDisplay(leave)}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              <p className="text-[12px] text-slate-500 truncate max-w-[120px] md:max-w-[150px]">
                                {leave.position}
                              </p>
                              <div className="w-1 h-1 rounded-full bg-gray-300"></div>
                              <span
                                className={`px-2 py-0.5 rounded-full text-[11px] font-bold whitespace-nowrap ${getBadgeStyle(leave.type || '')}`}
                              >
                                {leave.type}
                              </span>
                            </div>
