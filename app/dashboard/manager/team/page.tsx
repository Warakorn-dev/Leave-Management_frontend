...
import { Users, User, UserX, XCircle, ChevronDown, CheckCircle2, Clock, Paperclip } from "lucide-react";
...
export default function TeamDashboard() {
  ...
  useEffect(() => {
    const storedUsername = sessionStorage.getItem("fullName") || sessionStorage.getItem("username");
  ...
  if (isLoading) return <div className="p-8 text-black flex justify-center">กำลังโหลดข้อมูล...</div>;
    ...
    <div className="p-6 md:p-8 w-full space-y-6 text-black min-h-full bg-[#F8F9FA]">
      ...
      <div className="bg-[#0B0F4E] rounded-2xl p-8 flex flex-col md:flex-row items-start md:items-center justify-between text-white relative overflow-hidden shadow-sm">
        <div className="z-10">
          ...
          <p className="text-[#9EA1FF] text-[13px] font-medium mb-4">ยินดีต้อนรับสู่ Dashboard ของแผนกคุณ</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Card className="rounded-2xl border border-gray-200 shadow-sm bg-white hover:shadow-md transition-shadow">
          <CardContent className="p-5 flex flex-col items-start h-full justify-center">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-[13px] font-extrabold text-gray-700">พนักงานทั้งหมด</p>
            </div>
            <div className="flex items-baseline gap-1.5 mt-2 pl-1">
              <span className="text-[40px] font-black text-gray-800 leading-none tracking-tighter">{stats.totalEmployees}</span>
        ...
        <Card className="rounded-2xl border border-gray-200 shadow-sm bg-white hover:shadow-md transition-shadow">
          <CardContent className="p-5 flex flex-col items-start h-full justify-center">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-[13px] font-extrabold text-gray-700">จำนวนการลาวันนี้</p>
            </div>
            <div className="flex items-baseline gap-1.5 mt-2 pl-1">
              <span className="text-[40px] font-black text-gray-800 leading-none tracking-tighter">{stats.leavesToday}</span>
              <span className="text-[13px] font-bold text-gray-600">คน</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border border-gray-200 shadow-sm bg-white hover:shadow-md transition-shadow">
          <CardContent className="p-5 flex flex-col items-start h-full justify-center">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-[13px] font-extrabold text-gray-700">พนักงานคงเหลือ</p>
            </div>
            <div className="flex items-baseline gap-1.5 mt-2 pl-1">
              <span className="text-[40px] font-black text-gray-800 leading-none tracking-tighter">{stats.remainingEmployees}</span>
              <span className="text-[13px] font-bold text-gray-600">คน</span>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Pending Approvals */}
        <Card className="rounded-2xl border border-gray-200 shadow-sm bg-white hover:shadow-md transition-shadow cursor-pointer" onClick={() => window.location.href='/dashboard/manager/approve'}>
          <CardContent className="p-5 flex flex-col items-start h-full justify-center">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-8 h-8 rounded-full bg-amber-50 flex items-center justify-center">
                <Clock className="w-4 h-4 text-amber-500" />
              </div>
              <p className="text-[13px] font-extrabold text-gray-700">รออนุมัติ</p>
            </div>
            <div className="flex items-baseline gap-1.5 mt-2 pl-1">
              <span className="text-[40px] font-black text-gray-800 leading-none tracking-tighter">{stats.pendingApprovals || 0}</span>
              <span className="text-[13px] font-bold text-gray-600">รายการ</span>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              {topActivities.map((act: any, i: number) => (
                <div key={i} className="relative">
                  <div className={`absolute -left-[26px] top-1 w-3 h-3 rounded-full shadow-sm ${
                    ...
                    act.type === 'approve' ? 'bg-[#4CAF50]' : 'bg-[#2196F3]'
                  }`}></div>
