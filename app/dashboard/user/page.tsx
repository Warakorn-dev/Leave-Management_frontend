...
import { Calendar, Clock, CheckCircle2, XCircle, Plus, Paperclip } from "lucide-react";
...
export default function UserDashboard() {
  ...
  const [announcementYear, setAnnouncementYear] = useState(new Date().getFullYear());
  ...
  const { data: dashboardData, isLoading: isDashboardLoading } = useDashboardStats(selectedYear);

  useEffect(() => {
    const fullName = sessionStorage.getItem("fullName") || sessionStorage.getItem("username") || "ผู้ใช้งาน";
    ...
    if (dashboardData) {
      ...
      if (dashboardData.chartData && dashboardData.chartData.length === 12) {
        setChartData(dashboardData.chartData.map((d: any) => ({ month: d.name, value: d.value })));
      }
    }
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#F8F9FA] p-6 font-sans text-slate-800">
      ...
      <div className="bg-[#0B1038] rounded-xl p-8 flex flex-col md:flex-row items-start md:items-center justify-between mb-6 shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-blue-200/80 text-sm">"ยินดีต้อนรับสู่ แดชบอร์ด ของคุณ"</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <Card className="bg-white border-none shadow-sm rounded-xl overflow-hidden">
          <CardContent className="p-6">
            ...
            <p className="text-sm font-bold text-gray-500 mb-1">วันลาพักผ่อนประจำปี คงเหลือ</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-10">
              ...
              {chartData.map((data, index) => (
                <div key={index} className="flex flex-col items-center gap-2 w-full">
                  <div className="w-full max-w-[28px] bg-[#009DE0] rounded-sm relative group"
                       style={{ height: `${(data.value / dynamicMax) * 100}%`, minHeight: data.value > 0 ? '4px' : '0' }}>
                    {data.value > 0 && (
        ...
        <div className="lg:col-span-4 bg-[#0B1038] rounded-xl shadow-sm p-6 flex flex-col h-[340px]">
          ...
          <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            ...
            {announcements.filter(ann => new Date(ann.createdAt).getFullYear() === announcementYear).length > 0 ? (
              [...announcements]
                .filter(ann => new Date(ann.createdAt).getFullYear() === announcementYear)
                .sort((a, b) => {
                  if (a.isImportant === b.isImportant) return 0;
                  return a.isImportant ? -1 : 1;
                }).map((ann, idx) => (
                <div key={idx} className={`${ann.isImportant ? 'bg-[#3b82f6]/20 border-[#3b82f6]/50' : 'bg-[#2B3B7B] border-white/5'} p-4 rounded-xl border hover:bg-[#34468f] transition-colors cursor-pointer relative overflow-hidden`}>
                  {ann.isImportant && (
                  ...
                  <h3 className="font-bold text-white text-sm mb-1 pr-12">{ann.title}</h3>
                  <p className="text-xs text-blue-200/70 mb-2">{ann.subtitle}</p>
                  {ann.attachmentName && (
                    <a 
                      href="#" 
