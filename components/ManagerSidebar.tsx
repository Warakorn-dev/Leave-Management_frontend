...
export function ManagerSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const menuItems = [
    ...
    { name: "ตั้งค่าผู้ใช้", href: "/dashboard/manager/settings", icon: Settings },
  ];
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-4 space-y-3 overflow-y-auto mt-2">
        {menuItems.map((item) => {
          const isExactActive = pathname === item.href || (item.name === "Dashboard" && pathname === "/dashboard/manager/team");
